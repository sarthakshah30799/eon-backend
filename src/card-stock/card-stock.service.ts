...
export class CardStockService {
  constructor(
    ...
    @InjectRepository(CardStockReceipt, "database2")
    private readonly receiptRepository: Repository<CardStockReceipt>,
  ...
  async findAll(
    query?: CardStockReceiptListQueryDto,
    branchId?: string,
  ): Promise<PaginatedResponseDto<CardStockReceipt>> {
    ...
    if (branchId) qb.where("receipt.branchId = :branchId", { branchId });
    applyPagination(qb, pagination);
    const [receipts, total] = await qb.getManyAndCount();
    return buildPaginatedResponse(
      await this.withMaskedCards(receipts),
      total,
      pagination,
    );
  }
  ...
  async findById(
  ...
  ): Promise<CardStockReceipt & { printCount: number }> {
    const receipt = await this.receiptRepository.findOne({
      where: { id },
      relations: ["items", "items.cards"],
    });
    ...
    if (
      ...
      !session.isHo &&
      !session.isHoStaff &&
      receipt.branchId !== session.activeBranchId
    )
      throw new ForbiddenException(
        "CARD stock receipt does not belong to the current branch",
      );
  ...
  async recordPrint(
    id: string,
    dto: RecordCardStockPrintDto,
    session: AuthenticatedSession,
  ) {
    const receipt = await this.findById(id, session);
    if (!receipt.transactionId) {
      throw new BadRequestException(
        "CARD stock receipt has no technical transaction for print logging",
      );
    }
  ...
  async previewUpload(
    ...
    if (!file?.buffer?.length)
      throw new BadRequestException("CARD stock file is required");
    const issuer = issuerPartyProfileId
      ? await this.requireIssuer(issuerPartyProfileId)
      : null;
    ...
    return records.map((record, index) => {
      ...
      if (!String(record["kit number"] ?? "").trim())
      ...
      return {
        ...
        cardNumber:
          ...
          String(record["card number"] ?? "").trim(),
        denomination,
  ...
  async findAvailableCards(
    ...
    if (multiCurrency) {
      return this.database2.query(
        ...
        [branchId, productId, issuerPartyProfileId],
      );
    }
    ...
    return this.database2.query(
      ...
      ORDER BY c.series, c.kit_number`,
      [branchId, currencyId, productId, issuerPartyProfileId],
    );
  }

  async findReloadCards(
    branchId: string,
    passengerId: string,
    currencyId: string,
    productId: string,
    ...
    if (!branchId || !passengerId || !productId || !issuerPartyProfileId)
      return [];
    const product = await this.productRepository.findOne({
      where: { id: productId },
    });
    ...
    if (multiCurrency) {
      return this.database2.query(
        ...
        ORDER BY c.series, c.kit_number`,
        [branchId, productId, issuerPartyProfileId, passengerId],
      );
    }
    ...
    return this.database2.query(
      ...
      ORDER BY c.series, c.kit_number`,
      [branchId, currencyId, productId, issuerPartyProfileId, passengerId],
    );
  }

  async create(
    dto: CreateCardStockReceiptDto,
    session: AuthenticatedSession,
  ): Promise<CardStockReceipt> {
    ...
    if (
      !effectiveBranchId ||
      (!canSelectBranch && dto.branchId !== effectiveBranchId)
    )
      throw new BadRequestException(
        "Stock receipt must use the current branch",
      );
    ...
    const issuerSnapshot = await loadEntitySnapshot(
    ...
    const normalizedItems = await Promise.all(
      dto.items.map((item) => this.validateItem(item)),
    );
    const calculatedTotal = normalizedItems.reduce(
      (sum, item) => sum + item.feAmount,
      0,
    );
    if (Math.abs(calculatedTotal - Number(dto.totalFeAmount)) > 0.005)
      throw new BadRequestException(
        "Total FE amount does not match item totals",
      );
    ...
    const receipt = await this.database2.transaction(async (manager) => {
      ...
      const savedReceipt = await receiptRepo.save(
        receiptRepo.create({
          ...
          companySnapshot,
          issuerPartyProfileId: headerIssuer.id,
          issuerPartyProfileSnapshot: issuerSnapshot ?? {},
          status: CardStockReceiptStatus.POSTED,
          totalFeAmount: calculatedTotal.toFixed(2),
          createdBy: userId,
          updatedBy: userId,
        }),
      );
      ...
      for (const item of normalizedItems) {
        ...
        for (const card of item.cards) {
          ...
          if (seenCards.has(duplicateKey))
            throw new BadRequestException(
              `Duplicate kit/card number in receipt: ${card.kitNumber}`,
            );
          seenCards.add(duplicateKey);
          const existing = await manager.query(
            "SELECT id FROM card_stock_cards WHERE kit_number = $1 AND public.decrypt_card_number(card_number) = $2 LIMIT 1",
            [card.kitNumber, card.cardNumber],
          );
          if (existing.length)
            throw new BadRequestException(
              `Card already exists for kit number ${card.kitNumber}`,
            );
          ...
          await cardRepo.save(
            cardRepo.create({
              ...
              currentBranchSnapshot: branchSnapshot ?? {},
  ...
  private async requireActiveBranch(id: string): Promise<Branch> {
    ...
    if (!branch)
      throw new BadRequestException("A valid active branch is required");
    return branch;
  }
  ...
  private async requireIssuer(id: string): Promise<PartyProfile> {
    ...
    if (!issuer)
      throw new BadRequestException(
        "Card issuer profile is invalid, inactive, or not approved",
      );
    return issuer;
  }
  ...
  private async validateItem(item: CreateCardStockReceiptDto["items"][number]) {
    ...
    if (!currency)
      throw new BadRequestException(
        `Currency ${item.currencyId} is invalid or inactive`,
      );
    if (
      !product ||
      ...
      throw new BadRequestException(
        "Only active CARD products CC or CM are allowed",
      );
    }
    const multiCurrency = isMultiCurrencyCardProduct(product.productCode);
    if (multiCurrency) {
    ...
    if (!link)
      throw new BadRequestException(
        `Issuer ${issuer.code} is not configured for product ${product.productCode.toUpperCase()}`,
      );
    if (!item.cards?.length)
      throw new BadRequestException(
        `Item ${item.lineNo} must contain at least one card`,
      );
    const cards = item.cards.map((card) => {
      if (!/^[A-Za-z0-9]{1,4}$/.test(card.series))
        throw new BadRequestException(
          `Item ${item.lineNo}: series prefix must be 1 to 4 alphanumeric characters (for example, CC)`,
        );
      ...
      if (Number(card.amount) !== Number(card.denomination))
        throw new BadRequestException(
          `Item ${item.lineNo}: card amount is invalid`,
        );
      if (new Date(`${card.expirationDate}T00:00:00`) <= new Date())
        throw new BadRequestException(
          `Item ${item.lineNo}: expiration date must be in the future`,
        );
      const seriesPrefix = card.series.toUpperCase();
      return {
        ...card,
        series: `${seriesPrefix}0000`,
        cardNumber: cardNumberCheck.cardNumber,
        denomination: Number(card.denomination),
        amount: Number(card.amount),
      };
    });
    const feAmount = cards.reduce((sum, card) => sum + card.amount, 0);
    if (Math.abs(feAmount - Number(item.feAmount)) > 0.005)
      throw new BadRequestException(
        `Item ${item.lineNo}: FE amount does not match card totals`,
      );
    return {
      lineNo: item.lineNo,
      currencyId: currency.id,
      currencySnapshot:
        (await loadEntitySnapshot(this.currencyRepository, currency.id)) ?? {},
      per: Number(item.per),
      productId: product.id,
      productSnapshot:
        (await loadEntitySnapshot(this.productRepository, product.id)) ?? {},
      issuerPartyProfileId: issuer.id,
      issuerPartyProfileSnapshot:
        (await loadEntitySnapshot(this.partyProfileRepository, issuer.id)) ??
        {},
      feAmount,
      cards,
    };
  ...
  private async withMaskedCards(
    receipts: CardStockReceipt[],
  ): Promise<CardStockReceipt[]> {
    const cards = receipts.flatMap(
      (receipt) => receipt.items?.flatMap((item) => item.cards ?? []) ?? [],
    );
    ...
    const maskedById = new Map(
      rows.map((row: { id: string; maskedCardNumber: string }) => [
        row.id,
    ...
    return receipts.map((receipt) => ({
      ...
      items: receipt.items?.map((item) => ({
        ...
        cards: item.cards?.map((card) => ({
          ...
          maskedCardNumber: maskedById.get(card.id) ?? null,
        })),
      })),
    }));
  }
