...
export class CardTransferService {
  constructor(
    ...
    private readonly requestRepository: Repository<CardTransferRequest>,
    @InjectRepository(CardTransferRequestItem, "database2")
    private readonly itemRepository: Repository<CardTransferRequestItem>,
    @InjectRepository(CardTransferRequestCard, "database2")
    private readonly selectionRepository: Repository<CardTransferRequestCard>,
  ...
  private async validateItem(
    item: CardTransferItemDto,
    cardsById: Map<string, CardStockCard>,
  ) {
    const [currency, product, issuer] = await Promise.all([
      this.currencyRepository.findOne({
        where: { id: item.currencyId, active: true },
      }),
      this.productRepository.findOne({ where: { id: item.productId } }),
      this.partyProfileRepository.findOne({
        where: {
          id: item.issuerPartyProfileId,
          active: true,
    ...
    if (!currency)
      throw new BadRequestException(
        `Item ${item.lineNo}: currency is invalid or inactive`,
      );
    if (
      !product ||
      ...
      throw new BadRequestException(
        `Item ${item.lineNo}: only active CARD products CC or CM are allowed`,
      );
    if (!issuer)
      throw new BadRequestException(
    ...
    if (!link)
      throw new BadRequestException(
        `Item ${item.lineNo}: issuer is not configured for product ${product.productCode.toUpperCase()}`,
      );
    if (!item.cardIds?.length)
      throw new BadRequestException(
        `Item ${item.lineNo}: at least one card is required`,
      );

    const uniqueIds = new Set(item.cardIds);
    if (uniqueIds.size !== item.cardIds.length)
      throw new BadRequestException(
        `Item ${item.lineNo}: duplicate card selection is not allowed`,
      );
    let feAmount = 0;
    for (const id of item.cardIds) {
      ...
      if (!card)
        throw new BadRequestException(
          `Item ${item.lineNo}: selected card ${id} is unavailable`,
        );
      if (
        card.receiptItem?.currencyId !== currency.id ||
        card.receiptItem?.productId !== product.id ||
        card.receiptItem?.issuerPartyProfileId !== issuer.id
      ) {
        throw new BadRequestException(
          `Item ${item.lineNo}: selected card ${id} does not match currency, product, or issuer`,
        );
      }
  ...
  private async loadLockedCards(
    ...
    if (!ids.length)
      throw new BadRequestException(
        "At least one CARD stock record is required",
      );
    ...
    if (cards.length !== ids.length)
      throw new BadRequestException(
        "One or more selected CARD stock records were not found",
      );
    for (const card of cards) {
      if (sourceBranchId && card.currentBranchId !== sourceBranchId)
        throw new BadRequestException(
          "Selected card is not held by the source branch",
        );
      if (
        card.status !== CardStockCardStatus.AVAILABLE &&
        card.reservedByTransferId !== transferId
      )
        throw new BadRequestException(
          "One or more selected cards are already reserved or unavailable",
        );
    }
  ...
  async list(
    ...
    params?: CardTransferListQueryDto,
  ) {
    ...
    if (!this.isHoAccess(session)) {
      ...
      qb.andWhere(
        "(request.sourceBranchId = :branchId OR request.destinationBranchId = :branchId)",
    ...
    return buildPaginatedResponse(
      await this.withBranchObjects(rows),
      total,
      pagination,
    );
  }
  ...
  async findById(id: string, session: AuthenticatedSession) {
    const request = await this.requestRepository.findOne({
      where: { id },
      relations: { items: { selectedCards: { card: true } } },
    });
    if (!request)
      throw new NotFoundException("CARD transfer request not found");
    if (
      !this.isHoAccess(session) &&
      session.activeBranchId !== request.sourceBranchId &&
      session.activeBranchId !== request.destinationBranchId
    )
      throw new ForbiddenException(
        "You cannot view this CARD transfer request",
      );
    ...
    const [sourcePrintCount, destinationPrintCount] = await Promise.all([
      ...
      request.destinationTransactionId
        ? this.cardStockPrintService.countPrints(
            request.destinationTransactionId,
          )
        : Promise.resolve(0),
    ]);
  ...
  async recordPrint(
    id: string,
    dto: RecordCardTransferPrintDto,
    session: AuthenticatedSession,
  ) {
    ...
    if (
      ...
      throw new ForbiddenException(
        "You cannot print this CARD transfer request",
      );
    }
    if (request.status !== CardTransferStatus.ACCEPTED) {
      throw new BadRequestException(
        "Only accepted CARD transfers can be printed",
      );
    }
    if (dto.kind === CardStockPrintKind.STOCK_OUT) {
      ...
      if (!request.sourceTransactionId) {
        throw new BadRequestException(
          "CARD transfer has no source transaction for print logging",
        );
      }
    ...
    if (
      ...
      throw new ForbiddenException(
        "You can only print Stock In from the destination branch",
      );
    }
    if (!request.destinationTransactionId) {
      throw new BadRequestException(
        "CARD transfer has no destination transaction for print logging",
      );
    }
    return this.cardStockPrintService.recordPrint({
  ...
  async availableCards(sourceBranchId: string, session: AuthenticatedSession) {
    ...
    const numberById = new Map(
      decrypted.map((row: { id: string; cardNumber: string }) => {
        const value = String(row.cardNumber ?? "");
        return [
          row.id,
          value.length > 8
            ? `${value.slice(0, 4)}${"X".repeat(value.length - 8)}${value.slice(-4)}`
            : value,
        ];
      }),
    );
  ...
  async create(dto: CreateCardTransferDto, session: AuthenticatedSession) {
    ...
    if (new Set(flatIds).size !== flatIds.length)
      throw new BadRequestException(
        "A card can be selected only once in a transfer request",
      );
    const branchSnapshot = await loadEntitySnapshot(
      this.branchRepository,
      source.id,
    );
    const destinationSnapshot = await loadEntitySnapshot(
      this.branchRepository,
      destination.id,
    );
    ...
    const saved = await this.database2.transaction(async (manager) => {
      ...
      for (const item of dto.items) {
        const result = await this.validateItem(item, cardsById);
        total += result.feAmount;
        validated.push({ item, ...result });
      }
      ...
      const request = await requestRepo.save(
        requestRepo.create({
          ...
          destinationBranchSnapshot: destinationSnapshot ?? {},
          companyId: company.id,
          companySnapshot,
          status: CardTransferStatus.HELD,
          totalFeAmount: total.toFixed(2),
          remarks: dto.remarks?.trim() || null,
          heldAt: new Date(),
          heldById: session.userId,
      ...
      for (const row of validated) {
        const savedItem = await itemRepo.save(
          itemRepo.create({
            ...
            issuerPartyProfileId: row.issuer.id,
            issuerPartyProfileSnapshot:
              (await loadEntitySnapshot(
                this.partyProfileRepository,
                row.issuer.id,
              )) ?? {},
            feAmount: row.feAmount.toFixed(2),
            createdBy: session.userId,
            updatedBy: session.userId,
          }),
        );
        await selectionRepo.save(
          row.item.cardIds.map((cardId) =>
            selectionRepo.create({
              transferId: request.id,
              transferItemId: savedItem.id,
              cardId,
              createdBy: session.userId,
              updatedBy: session.userId,
            }),
      ...
      for (const card of cardsById.values()) {
        ...
        card.reservedByTransferId = request.id;
        card.reservedAt = new Date();
        card.updatedBy = session.userId;
      }
    ...
    await this.notifyBranchUsers(
      destination.id,
      `CARD transfer ${transactionNumber} submitted`,
      `CARD transfer request ${transactionNumber} is awaiting acceptance.`,
    );
  ...
  async update(
    ...
    const saved = await this.database2.transaction(async (manager) => {
      const request = await manager
        .getRepository(CardTransferRequest)
        .createQueryBuilder("request")
      ...
      const oldSelections = await manager
        .getRepository(CardTransferRequestCard)
        .find({ where: { transferId: id } });
      ...
      if (oldIds.length) {
        await manager
          ...
          .where("reserved_by_transfer_id = :id", { id })
          .execute();
      }
      const cardsById = await this.loadLockedCards(manager, flatIds, source.id);
      const validated = [];
  ...
  async accept(id: string, session: AuthenticatedSession) {
    const request = await this.requestRepository.findOne({
      where: { id },
      relations: { items: { selectedCards: true } },
    });
    if (!request)
      throw new NotFoundException("CARD transfer request not found");
    if (request.status !== CardTransferStatus.HELD)
      throw new BadRequestException("Only held CARD transfers can be accepted");
    if (
      !this.isHoAccess(session) &&
      session.activeBranchId !== request.destinationBranchId
    )
      throw new ForbiddenException(
        "Only the destination branch or Admin/HO can accept this request",
      );
    const [source, destination] = await Promise.all([
      this.branchRepository.findOne({
        where: { id: request.sourceBranchId, isActive: true },
      }),
      this.branchRepository.findOne({
    ...
    await this.database2.transaction(async (manager) => {
      const locked = await manager
        ...
        .setLock("pessimistic_write")
        .getOne();
      if (!locked || locked.status !== CardTransferStatus.HELD)
        throw new BadRequestException(
          "CARD transfer has already been processed",
        );
      const selections = await manager
        .getRepository(CardTransferRequestCard)
        .find({
          where: { transferId: id },
          relations: ["card", "card.receiptItem", "transferItem"],
        });
      const cards = await this.loadLockedCards(
        manager,
        selections.map((selection) => selection.cardId),
      ...
      const transactionItems = selections.map((selection) => ({
        ...
        cardSnapshot: {
          ...
          denomination: selection.card.denomination,
          amount: selection.card.amount,
          expirationDate: selection.card.expirationDate,
        },
      }));
      for (const card of cards.values()) {
        card.currentBranchId = destination.id;
        card.currentBranchSnapshot = ((await loadEntitySnapshot(
          this.branchRepository,
          destination.id,
        )) ?? { id: destination.id }) as TransactionReferenceSnapshot;
        card.status = CardStockCardStatus.AVAILABLE;
      ...
      const transferOutTransaction =
        await this.cardStockTransactionService.create({
          manager,
          operationCode: TransactionTypeProfileEnum.CARD_TRANSFER_OUT,
          branch: source,
          branchSnapshot: request.sourceBranchSnapshot,
          companyId: request.companyId,
          companySnapshot: request.companySnapshot,
          transactionDate: request.transactionDate,
          referenceType: CardStockReferenceType.CARD_TRANSFER_REQUEST,
      ...
      const transferInTransaction =
        await this.cardStockTransactionService.create({
          manager,
          operationCode: TransactionTypeProfileEnum.CARD_TRANSFER_IN,
          branch: destination,
          branchSnapshot: request.destinationBranchSnapshot,
          companyId: request.companyId,
          companySnapshot: request.companySnapshot,
          transactionDate: request.transactionDate,
          referenceType: CardStockReferenceType.CARD_TRANSFER_REQUEST,
      ...
      locked.status = CardTransferStatus.ACCEPTED;
      locked.acceptedAt = new Date();
      locked.acceptedById = session.userId;
      locked.acceptanceRemarks = null;
      locked.sourceTransactionId = transferOutTransaction.id;
      locked.destinationTransactionId = transferInTransaction.id;
      locked.updatedBy = session.userId;
      await manager.getRepository(CardTransferRequest).save(locked);
    });
    await this.notifyBranchUsers(
      request.sourceBranchId,
      `CARD transfer ${request.transactionNumber} accepted`,
      `CARD transfer request ${request.transactionNumber} was accepted by the destination branch.`,
    );
    return this.findById(id, session);
  }

  async reject(id: string, remarks: string, session: AuthenticatedSession) {
  ...
  private async finishHeld(
    id: string,
    status:
      | typeof CardTransferStatus.REJECTED
      | typeof CardTransferStatus.CANCELLED,
    remarks: string,
    session: AuthenticatedSession,
  ) {
    if (!remarks?.trim())
      throw new BadRequestException(
        `${status === CardTransferStatus.REJECTED ? "Rejection" : "Cancellation"} reason is required`,
      );
    const current = await this.requestRepository.findOne({ where: { id } });
    if (!current)
      throw new NotFoundException("CARD transfer request not found");
    if (current.status !== CardTransferStatus.HELD)
      throw new BadRequestException("Only held CARD transfers can be changed");
    if (
      status === CardTransferStatus.REJECTED &&
      !this.isHoAccess(session) &&
      session.activeBranchId !== current.destinationBranchId
    )
      throw new ForbiddenException(
        "Only the destination branch or Admin/HO can reject this request",
      );
    await this.database2.transaction(async (manager) => {
      const request = await manager
        .getRepository(CardTransferRequest)
        ...
        .setLock("pessimistic_write")
        .getOne();
      if (!request || request.status !== CardTransferStatus.HELD)
        throw new BadRequestException(
          "CARD transfer has already been processed",
        );
      ...
      request.updatedBy = session.userId;
      if (status === CardTransferStatus.REJECTED) {
        request.rejectedAt = new Date();
        request.rejectedById = session.userId;
        request.rejectionReason = remarks.trim();
      } else {
        request.cancelledAt = new Date();
        request.cancelledById = session.userId;
        request.cancellationReason = remarks.trim();
      }
      await manager.getRepository(CardTransferRequest).save(request);
    });
    await this.notifyBranchUsers(
      current.sourceBranchId,
      `CARD transfer ${current.transactionNumber} ${status.toLowerCase()}`,
      `CARD transfer request ${current.transactionNumber} was ${status.toLowerCase()}. Reason: ${remarks.trim()}`,
    );
    if (status === CardTransferStatus.CANCELLED) {
      await this.notifyBranchUsers(
        current.destinationBranchId,
        `CARD transfer ${current.transactionNumber} cancelled`,
        `CARD transfer request ${current.transactionNumber} was cancelled. Reason: ${remarks.trim()}`,
      );
    }
    return this.findById(id, session);
  }
  ...
  private async cancelReservationsWithManager(
    manager: import("typeorm").EntityManager,
    id: string,
    userId: string,
  ) {
  ...
  private async withBranchObjects(requests: CardTransferRequest[]) {
    ...
    const cardIds = requests.flatMap((request) =>
      (request.items ?? []).flatMap((item) =>
        (item.selectedCards ?? []).map((selection) => selection.cardId),
      ),
    );
