...
export class CardStockSettlementService {
  ...
  constructor(
    ...
    @InjectRepository(CardStockSettlementDocument, 'database2') private readonly documentRepository: Repository<CardStockSettlementDocument>,
  ...
  private parseRate(value: string, message: string) {
    ...
    if (!Number.isFinite(rate) || rate <= 0) throw new BadRequestException(message);
    return rate;
  }
  ...
  private async reserveNumber(branch: Branch, date: Date) {
    return this.additionalSettingService.reserveTransactionNumber(TransactionTypeProfileEnum.CARD_SETTLE, branch.code, date);
  }
  ...
  private async lockRowsById(manager: EntityManager, table: 'transactions' | 'transaction_items' | 'card_stock_settlements', ids: string[]) {
    for (const id of this.sortIds(ids)) {
      await manager.query(`SELECT 1 FROM ${table} WHERE id = $1 FOR UPDATE`, [id]);
    }
  }
  ...
  private async lockSettlementRows(manager: EntityManager, ids: string[]) {
    ...
    const preview: Array<{ id: string; transaction_id: string; transaction_item_id: string }> = await manager.query(
      `SELECT id, transaction_id, transaction_item_id FROM card_stock_settlements WHERE id = ANY($1::uuid[])`,
      [uniqueIds],
    );
    await this.lockRowsById(manager, 'transactions', preview.map(row => row.transaction_id));
    await this.lockRowsById(manager, 'transaction_items', preview.map(row => row.transaction_item_id));
    await this.lockRowsById(manager, 'card_stock_settlements', preview.map(row => row.id));
    const rows = await manager.getRepository(CardStockSettlement).find({ where: { id: In(uniqueIds) } });
  ...
  private async createDocument(
    ...
    return repo.save(repo.create({
      transactionNumber: await this.reserveNumber(input.numberBranch, input.transactionDate),
  ...
  async createForApprovedSale(manager: EntityManager, transaction: Transaction, items: TransactionItem[], actorId: string) {
    ...
    for (const item of items) {
      ...
      if (!row) {
        ...
        const saleDate = this.toTimestamp(transaction.transactionDate, `CARD item ${item.lineNo} is missing a sale date`);
        row = await repo.save(repo.create({
          ...
          saleBuyRate, buyRate: saleBuyRate, buyRateSnapshot: quote.snapshot, settlementAmount: (denomination * quote.buyRate).toFixed(2), saleDate,
          settlementMode: mode, saleKind: item.isReload ? CardStockSettlementSaleKind.RELOAD : CardStockSettlementSaleKind.FRESH,
          branchRequestedDate: freezeBranch ? saleDate : null, branchReference: null, branchRemarks: null,
          branchRequestedAt: freezeBranch ? new Date() : null, branchRequestedById: freezeBranch ? actorId : null,
          status: freezeBranch && sellingBranch.id !== ho.id ? CardStockSettlementStatus.PENDING_HO_ACCEPTANCE : CardStockSettlementStatus.PENDING_BRANCH_SETTLEMENT,
          createdBy: actorId, updatedBy: actorId,
        }));
      }
      saleRows.push(row);
    }
    const frozenRows = saleRows.filter(row => row.settlementMode === CardStockSettlementMode.AUTO && !row.branchDocumentId);
    if (frozenRows.length) await this.createFrozenBranchDocuments(manager, frozenRows, actorId);
  }

  private async createFrozenBranchDocuments(manager: EntityManager, rows: CardStockSettlement[], actorId: string) {
    ...
    for (const group of groups.values()) {
      ...
      const date = this.toTimestamp(first.branchRequestedDate ?? first.saleDate, 'CARD settlement date is invalid');
      ...
      for (const row of group) await itemRepo.update(row.id, { branchDocumentId: document.id, updatedBy: actorId });
      if (selling.id === ho.id) await this.postBranchDocument(manager, document.id, actorId, false);
    }
  }

  private async postBranchDocument(manager: EntityManager, documentId: string, actorId: string, acceptedByHo: boolean) {
    ...
    const items = await this.lockSettlementRows(manager, itemIds.map(row => row.id));
    if (!items.length) throw new BadRequestException('CARD settlement has no items');
    const branch = await this.getBranch(document.branchId);
    await this.dayEndStartProcessService.assertTransactionDateAllowed(branch.id, actorId, document.transactionDate);
    ...
    const postedItems = await itemRepo.find({ where: { branchDocumentId: document.id } });
    if (postedItems.some(row => !row.branchSettlementEntryId)) {
      throw new BadRequestException('Branch settlement ledger entry was not created for one or more cards');
    }
  ...
  private async postIssuerDocument(manager: EntityManager, documentId: string, actorId: string) {
    const documentRepo = manager.getRepository(CardStockSettlementDocument);
    const itemRepo = manager.getRepository(CardStockSettlement);
    const document = await documentRepo.findOne({ where: { id: documentId } });
    if (!document) throw new NotFoundException('CARD settlement not found');
    ...
    await this.dayEndStartProcessService.assertTransactionDateAllowed(ho.id, actorId, document.transactionDate);
    const posting = await this.cardStockTransactionService.create({
      manager,
      operationCode: TransactionTypeProfileEnum.CARD_SETTLE,
      number: document.transactionNumber,
    ...
    const postedItems = await itemRepo.find({ where: { issuerDocumentId: document.id } });
    if (postedItems.some(row => !row.issuerSettlementEntryId)) {
      throw new BadRequestException('Issuer settlement ledger entry was not created for one or more cards');
    }
    await itemRepo.update({ id: In(postedItems.map(row => row.id)) }, {
  ...
  async create(dto: CreateCardStockSettlementDocumentDto, session: AuthenticatedSession) {
    if (!session?.userId) throw new ForbiddenException('User session is required');
    const isHo = this.isHo(session);
    if (dto.kind === CardStockSettlementDocumentKind.HO_ISSUER && !isHo) throw new ForbiddenException('Only Admin/HO users can settle with issuers');
    if (dto.kind === CardStockSettlementDocumentKind.BRANCH_HO && isHo) throw new ForbiddenException('HO creates issuer settlements only');
    const id = await this.database2.transaction(async manager => {
      const itemRepo = manager.getRepository(CardStockSettlement);
      const uniqueIds = [...new Set(dto.items.map(item => item.id))];
      if (uniqueIds.length !== dto.items.length) throw new BadRequestException('Duplicate CARD items cannot be settled together');
      const rows = await this.lockSettlementRows(manager, uniqueIds);
      if (rows.length !== uniqueIds.length) throw new BadRequestException('One or more CARD items were not found');
      const rates = new Map(dto.items.map(item => [item.id, this.parseRate(item.rate, 'Settlement rate must be greater than zero')]));
      const first = rows[0];
      if (rows.some(row => row.issuerPartyProfileId !== dto.issuerPartyProfileId || row.currencyId !== dto.currencyId)) {
        throw new BadRequestException('Selected CARD items must match the issuer and currency');
      }
      const date = this.toTimestamp(dto.transactionDate, 'Settlement date is invalid');
      if (rows.some(row => this.isCalendarBefore(date, row.saleDate))) throw new BadRequestException('Settlement date cannot be before the CARD sale date');
      ...
      if (dto.kind === CardStockSettlementDocumentKind.BRANCH_HO) {
        ...
        if (!branchId) throw new BadRequestException('Current branch is required');
        if (rows.some(row => row.branchId !== branchId || row.status !== CardStockSettlementStatus.PENDING_BRANCH_SETTLEMENT || row.settlementMode !== CardStockSettlementMode.MANUAL || row.branchDocumentId)) {
          throw new BadRequestException('Only unsettled MANUAL CARD items for the current branch can be submitted');
        }
        const document = await this.createDocument(manager, {
          kind: CardStockSettlementDocumentKind.BRANCH_HO,
          status: CardStockSettlementDocumentStatus.PENDING_HO_ACCEPTANCE,
      ...
      if (rows.some(row => row.status !== CardStockSettlementStatus.PENDING_ISSUER_SETTLEMENT || !row.branchSettlementEntryId || row.issuerDocumentId)) {
        throw new BadRequestException('Only branch-settled CARD items can be settled with issuers');
      }
      if (!dto.hoBranchId) throw new BadRequestException('HO branch is required');
      const ho = await this.getBranch(dto.hoBranchId);
      if (!ho.isHeadOffice) throw new BadRequestException('Issuer settlement must use an HO branch');
      if (rows.some(row => row.hoBranchId !== ho.id)) {
        throw new BadRequestException('Selected CARD items must belong to the selected HO branch');
      }
      if (rows.some(row => this.isCalendarBefore(date, row.branchSettlementDate ?? row.saleDate))) {
        throw new BadRequestException('Issuer settlement date cannot be before the branch settlement date');
      }
      const document = await this.createDocument(manager, {
        ...
        currencySnapshot: first.currencySnapshot,
        branchId: ho.id,
        branchSnapshot: { id: ho.id, code: ho.code, name: ho.name, label: `${ho.code} - ${ho.name}` },
        hoBranchId: ho.id,
        hoBranchSnapshot: { id: ho.id, code: ho.code, name: ho.name, label: `${ho.code} - ${ho.name}` },
        reference,
        remarks,
        actorId: session.userId,
      });
  ...
  async listUnsettled(query: CardStockUnsettledQueryDto, session: AuthenticatedSession) {
    if (!session?.userId) throw new ForbiddenException('User session is required');
    const conditions = ['s.deleted_at IS NULL', 's.issuer_party_profile_id = $1', 's.currency_id = $2'];
    const params: unknown[] = [query.issuerPartyProfileId, query.currencyId];
    if (query.kind === CardStockSettlementDocumentKind.BRANCH_HO) {
      if (this.isHo(session)) throw new ForbiddenException('HO creates issuer settlements only');
      if (!session.activeBranchId) return [];
      params.push(session.activeBranchId);
      conditions.push(`s.branch_id = $${params.length}`, `s.status = 'PENDING_BRANCH_SETTLEMENT'`, `s.settlement_mode = 'MANUAL'`, 's.branch_document_id IS NULL');
    } else {
      if (!this.isHo(session)) throw new ForbiddenException('Only Admin/HO users can settle with issuers');
      if (!query.hoBranchId) throw new BadRequestException('HO branch is required');
      params.push(query.hoBranchId);
      conditions.push(`s.status = 'PENDING_ISSUER_SETTLEMENT'`, 's.issuer_document_id IS NULL', 's.branch_settlement_entry_id IS NOT NULL', `s.ho_branch_id = $${params.length}`);
    }
    return this.database2.query(
      `SELECT s.id, s.series, s.denomination, s.sale_kind AS "saleKind", s.sale_buy_rate AS "saleBuyRate", s.buy_rate AS "buyRate", s.settlement_amount AS "settlementAmount",
       ...
       WHERE ${conditions.join(' AND ')} ORDER BY s.sale_date DESC, s.created_at DESC`,
      params,
    );
  }

  async list(query: CardStockSettlementDocumentQueryDto, session: AuthenticatedSession) {
    ...
    if (query.issuerPartyProfileId) add('d.issuer_party_profile_id = ?', query.issuerPartyProfileId);
    if (query.currencyId) add('d.currency_id = ?', query.currencyId);
    if (query.branchId && this.isHo(session)) add('d.branch_id = ?', query.branchId);
    if (query.dateFrom) add('d.transaction_date >= ?', this.toTimestamp(query.dateFrom, 'Date from is invalid'));
    if (query.dateTo) add('d.transaction_date < ?', toUtcNextDate(query.dateTo) ?? this.toTimestamp(query.dateTo, 'Date to is invalid'));
    return this.database2.query(
      `SELECT d.id, d.transaction_number AS "transactionNumber", d.transaction_date AS "transactionDate", d.kind, d.status,
       ...
       FROM card_stock_settlement_documents d WHERE ${conditions.join(' AND ')} ORDER BY d.transaction_date DESC, d.created_at DESC`,
      params,
    );
  }

  async get(id: string, session: AuthenticatedSession) {
    const documents = await this.list({}, session);
    const document = documents.find((row: { id: string }) => row.id === id);
    if (!document) throw new NotFoundException('CARD settlement not found');
  ...
  async accept(id: string, session: AuthenticatedSession) {
    ...
    const documentId = await this.database2.transaction(async manager => {
      const documentRepo = manager.getRepository(CardStockSettlementDocument);
      const document = await documentRepo.createQueryBuilder('d').where('d.id = :id', { id }).setLock('pessimistic_write').getOne();
      if (!document) throw new NotFoundException('CARD settlement not found');
      if (document.kind !== CardStockSettlementDocumentKind.BRANCH_HO || document.status !== CardStockSettlementDocumentStatus.PENDING_HO_ACCEPTANCE || document.postingTransactionId) {
        throw new BadRequestException('Only pending HO acceptance settlements can be accepted');
      }
  ...
  async reject(id: string, dto: RejectCardStockSettlementDocumentDto, session: AuthenticatedSession) {
    this.assertHo(session);
    await this.database2.transaction(async manager => {
      ...
      if (!document) throw new NotFoundException('CARD settlement not found');
      if (document.kind !== CardStockSettlementDocumentKind.BRANCH_HO || document.status !== CardStockSettlementDocumentStatus.PENDING_HO_ACCEPTANCE || document.postingTransactionId) {
        throw new BadRequestException('Only pending HO acceptance settlements can be rejected');
      }
      const itemIds: Array<{ id: string }> = await manager.query(
        `SELECT id FROM card_stock_settlements WHERE deleted_at IS NULL AND branch_document_id = $1`,
        [document.id],
      ...
      for (const row of items) {
        await itemRepo.update(row.id, {
          ...
          settlementAmount: this.amountFrom(row.denomination, Number(row.saleBuyRate)),
  ...
  async cancel(id: string, dto: CancelCardStockSettlementDocumentDto, session: AuthenticatedSession) {
    if (!session?.userId) throw new ForbiddenException('User session is required');
    await this.database2.transaction(async manager => {
      const documentRepo = manager.getRepository(CardStockSettlementDocument);
      const itemRepo = manager.getRepository(CardStockSettlement);
      const document = await documentRepo.createQueryBuilder('d').where('d.id = :id', { id }).setLock('pessimistic_write').getOne();
      if (!document) throw new NotFoundException('CARD settlement not found');
      if (document.postingTransactionId || document.status !== CardStockSettlementDocumentStatus.PENDING_HO_ACCEPTANCE || document.kind !== CardStockSettlementDocumentKind.BRANCH_HO) {
        throw new BadRequestException('Only unposted branch settlements can be cancelled');
      }
      if (!this.isHo(session) && document.branchId !== session.activeBranchId) throw new ForbiddenException('Settlement must belong to the current branch');
      const itemIds: Array<{ id: string }> = await manager.query(
        `SELECT id FROM card_stock_settlements WHERE deleted_at IS NULL AND branch_document_id = $1`,
        [document.id],
      );
