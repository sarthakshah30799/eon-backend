...
export const resolveCardNumberLength = (length?: number | null) => {
  ...
  if (Number.isInteger(parsed) && parsed >= MIN_CARD_NUMBER_LENGTH && parsed <= MAX_CARD_NUMBER_LENGTH) {
    return parsed;
  }
  return DEFAULT_CARD_NUMBER_LENGTH;
};

export const normalizeCardNumber = (value?: string | null) => (value ?? '').replace(/\s/g, '').toUpperCase();
...
export const validateCardNumber = (value: string, rule: CardNumberRule = {}, requireMask = false) => {
  ...
  if (requireMask && allowMasking) {
    return isMasked
      ...
      : { valid: false as const, cardNumber, message: `Card number must be a ${length}-character mask` };
  }
  if (isDigits) {
  ...
  return { valid: false as const, cardNumber, message: `Card number must be ${length} digits` };
};
