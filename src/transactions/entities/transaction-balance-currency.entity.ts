...
@Index("IDX_transaction_balance_currencies_bucket", ["date", "branchId", "counterId", "currencyId", "profileType"], {
  unique: true,
})
@Index("IDX_transaction_balance_currencies_branch_counter_date", ["branchId", "counterId", "date"])
...
export class TransactionBalanceCurrency extends BaseEntity {
  ...
  @Column({ type: "numeric", name: "openingrs", precision: 18, scale: 2, default: 0 })
  openingRs: string;

  @Column({ type: "numeric", precision: 18, scale: 7, default: 0 })
  purchase: string;

  @Column({ type: "numeric", name: "purchasers", precision: 18, scale: 2, default: 0 })
  purchaseRs: string;

  @Column({ type: "numeric", precision: 18, scale: 7, default: 0 })
  sell: string;

  @Column({ type: "numeric", name: "sellrs", precision: 18, scale: 2, default: 0 })
  sellRs: string;

  @Column({ type: "numeric", name: "adjustsellrs", precision: 18, scale: 2, default: 0 })
  adjustSellRs: string;

  @Column({ type: "numeric", precision: 18, scale: 7, default: 0 })
  closing: string;

  @Column({ type: "numeric", name: "closingrs", precision: 18, scale: 2, default: 0 })
  closingRs: string;
}
