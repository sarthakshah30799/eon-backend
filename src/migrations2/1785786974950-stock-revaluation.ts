...
export class StockRevaluation1785786974950 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP FUNCTION IF EXISTS public.calculate_stock_revaluation(uuid, uuid, date, jsonb)`,
    );
    await queryRunner.query(
      `ALTER TABLE "stock_revaluation_items" DROP CONSTRAINT "FK_stock_revaluation_items_revaluation"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_stock_revaluation_items_currency"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_stock_revaluation_items_revaluation_line"`,
    );
    await queryRunner.query(`DROP TABLE "stock_revaluation_items"`);
    await queryRunner.query(
      `DROP INDEX "public"."IDX_stock_revaluations_branch_date"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_stock_revaluations_branch_counter_period"`,
    );
    await queryRunner.query(`DROP TABLE "stock_revaluations"`);
    await queryRunner.query(
      `DROP TYPE "public"."stock_revaluation_frequency_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE "public"."stock_revaluation_status_enum"`,
    );
  }
}
