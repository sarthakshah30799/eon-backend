...
export class TaxSummaryEntity1784705844391 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."transaction_account_postings_source_type_enum" AS ENUM('ITEM', 'ITEM_PROFIT', 'ITEM_SALE', 'ROUND_OFF', 'PARTY_CONTROL', 'ADDITIONAL_CHARGE', 'TAX_ITEM', 'TAX_ADDITIONAL_CHARGE', 'PAYMENT')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "transaction_tax_summaries" ADD CONSTRAINT "FK_transaction_tax_summaries_transaction_id" FOREIGN KEY ("transaction_id") REFERENCES "transactions"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "transaction_tax_summaries" DROP CONSTRAINT "FK_transaction_tax_summaries_transaction_id"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."transaction_account_postings_source_type_enum_old" AS ENUM('ADDITIONAL_CHARGE', 'ITEM', 'ITEM_PROFIT', 'ITEM_SALE', 'PARTY_CONTROL', 'PAYMENT', 'ROUND_OFF')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "transactions" DROP COLUMN "tax_rate_percent"`,
    );
