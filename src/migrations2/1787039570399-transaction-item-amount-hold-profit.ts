...
export class TransactionItemAmountHoldProfit1787039570399 implements MigrationInterface {
  name = "TransactionItemAmountHoldProfit1787039570399";
      ...
      BEGIN
        ...
        SET
          amount = CASE
            ...
            ELSE ROUND(ROUND(quantity * rate / COALESCE(NULLIF(per, 0), 1), 2), 0)
          END,
          round_off = CASE
            WHEN quantity IS NULL OR rate IS NULL THEN NULL
            ELSE ROUND(
              ROUND(ROUND(quantity * rate / COALESCE(NULLIF(per, 0), 1), 2), 0)
