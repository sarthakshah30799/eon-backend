...
export class TtDealOpsTables1790103808616 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "tt_remittance_details" ADD CONSTRAINT "FK_tt_remittance_details_transaction" FOREIGN KEY ("transaction_id") REFERENCES "transactions"("id") ON DELETE RESTRICT ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "tt_remittance_details" DROP CONSTRAINT "FK_tt_remittance_details_transaction"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."manual_books_transaction_type_enum_old" AS ENUM('FAKE_CURRENCY', 'PURCHASE_FFMC', 'PURCHASE_CORPORATE_INDIVIDUAL', 'SALE_CORPORATE_INDIVIDUAL', 'SALE_FFMC', 'SALE_RMC', 'SALE_FOREX', 'SALE_FOREIGN', 'SALE_MISC', 'SALE_FRANCHISE', 'PURCHASE_RMC', 'PURCHASE_FOREX', 'PURCHASE_FOREIGN', 'PURCHASE_MISC', 'PURCHASE_FRANCHISE')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "transaction_items" DROP COLUMN "tt_remittance_detail_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "transaction_items" DROP COLUMN "deal_cover_snapshot"`,
    );
    await queryRunner.query(
      `ALTER TABLE "transaction_items" DROP COLUMN "deal_cover_id"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."UQ_tt_remittance_details_transaction"`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX "public"."IDX_deal_covers_transaction_date"`,
    );
