...
export class StockRevaluationController {
  ...
  async template(@Res() response: Response, @Session() session: any) {
    ...
    response.setHeader(
      "Content-Disposition",
      'attachment; filename="stock-revaluation-template.xlsx"',
    );
  ...
  async current(
    ...
    @Query("frequency") frequency: StockRevaluationFrequency,
    @Session() session: any,
  ) {
        ...
        : [
            {
              ...
              counterId: session?.activeCounterId,
            },
          ];
    if (
      effectiveTargets.some((target) => !target.branchId || !target.counterId)
    )
      throw new BadRequestException("Active branch and counter are required");
    if (!Object.values(StockRevaluationFrequency).includes(frequency))
  ...
  @Post("process")
  async process(
    @Body() body: { targets?: string; frequency?: string },
    @Session() session: any,
  ) {
    ...
    if (
      !Object.values(StockRevaluationFrequency).includes(
        body.frequency as StockRevaluationFrequency,
      )
    ) {
      throw new BadRequestException("Stock revaluation frequency is invalid");
    }
    return this.service.process(
      {
        branchIds: [],
        frequency: body.frequency as StockRevaluationFrequency,
        rates: [],
      },
      session.userId,
      effectiveTargets,
  ...
  async upload(
    ...
    if (!file)
      throw new BadRequestException(
        "Stock revaluation template file is required",
      );
    const parsedTargets = this.parseTargets(targets);
    if (
      !Object.values(StockRevaluationFrequency).includes(
        frequency as StockRevaluationFrequency,
      )
    )
      throw new BadRequestException("Stock revaluation frequency is invalid");
    return this.service.upload(
      {
        branchIds: [],
        frequency: frequency as StockRevaluationFrequency,
        rates: [],
      },
      file,
      session.userId,
  ...
  private requireManager(session: any) {
    if (!session?.isAdmin && !session?.isHoStaff)
      throw new ForbiddenException(
        "Only Admin, HO, and HO Staff can upload, download, or remove stock revaluation rates",
      );
  }

  private parseTargets(
    value: string,
  ): Array<{ branchId: string; counterId: string }> {
    let parsed: unknown;
    try {
    ...
    if (
      ...
      parsed.some(
        (target) =>
          ...
          typeof target.counterId !== "string",
      )
    ) {
      throw new BadRequestException(
        "At least one branch and counter is required",
      );
    }
