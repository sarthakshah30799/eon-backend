...
import { TransactionDocumentContext } from '../transactions/transactions.enums';
...
export const STATIC_SELECT_OPTIONS: Record<string, StaticSelectOption[]> = {
  [CategoryOptionCodeEnum.MasterDocument]: toStaticOptions(Object.values(ClientType)),
  [CategoryOptionCodeEnum.TransactionDocument]: toStaticOptions(
    Object.values(TransactionDocumentContext),
  ),
};
...
export const getStaticSelectOptions = (
  ...
  const normalizedCode = code.trim().replace(/[_\s-]/g, '').toUpperCase();
