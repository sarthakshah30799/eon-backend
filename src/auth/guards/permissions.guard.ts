...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (isFakeCurrencyDraft) {
      ...
      const hasAnyRequestedPermission = requestedMenuPaths.some(
        (requestedMenuPath) =>
          (userDto.permissions?.[requestedMenuPath] || []).includes(
            requiredPermission,
          ),
      );
      ...
      if (!hasAnyRequestedPermission) {
        ...
        if (method === "GET" && !request.params?.id) {
          request.silentEmptyResult = this.resolveSilentEmptyResult(
            path,
            requestedMenuPaths[0],
          );
        ...
        throw new NotFoundException(
          `Resource at ${requestedMenuPaths[0]} not found`,
        );
      }
    ...
    const permissionPaths = allowedMenuPaths?.length
      ? allowedMenuPaths
      : [menuPath];
    const hasPermission = permissionPaths.some((permissionPath) =>
      (userDto.permissions?.[permissionPath] || []).includes(
        requiredPermission,
      ),
    );
    if (!hasPermission) {
  ...
  private resolveSilentEmptyResult(
    ...
    if (
      arrayPathMatches.some(
        (token) => path.includes(token) || menuPath.includes(token),
      )
    ) {
      return { kind: "array" };
    }
