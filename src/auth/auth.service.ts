...
export class AuthService {
  ...
  async validateOtpUser(verifyOtpDto: {
    countryCode: string;
    mobileNumber: string;
    otp: string;
  }): Promise<User | null> {
    return this.userService.validateOtpUser(
  ...
  async login(
    ...
    const isHoStaff =
      user.userRoles?.some((userRole) => userRole.role?.isHoStaff) === true;
    ...
    if (!user.isAdmin && !isHoStaff) {
      ...
      if (!hasAssignment) {
        throw new BadRequestException(
          "Selected branch and counter are not assigned to this user",
        );
      }
    }
  ...
  async completeInitialPasswordSetup(
    session: any,
    newPassword: string,
    ...
    if (!user.mustChangePassword) {
      throw new BadRequestException(
        "Password setup is not required for this user",
      );
    }
  ...
  async forgotPassword(email: string): Promise<{ message: string }> {
    ...
    try {
      ...
      console.warn(
        "Failed to send reset email via SMTP, printing to console as fallback:",
        error.message,
      );
      console.log(
        "\n============================================================",
