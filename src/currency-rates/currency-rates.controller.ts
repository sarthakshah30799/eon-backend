...
export class CurrencyRatesController {
  ...
  async preview(
    ...
    return CurrencyRateQuoteResponseDto.fromValue(
      await this.service.previewQuote(dto),
    );
  }
  ...
  async getProductCurrencyRules(
    ...
    const rules = await this.service.findProductCurrencyRates(
      productId,
      currencyId,
    );
    return rules.map((rule) =>
      Object.assign(new ProductCurrencyRateResponseDto(), rule),
    );
  }
  ...
  async getContext(
    ...
    const context = await this.service.getCurrencyRateContext(
      productId,
      currencyId,
    );
    return {
