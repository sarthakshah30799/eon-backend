...
const normalizeGroupCode = (code?: string | null) =>
  String(code ?? "")
    .trim()
    .toUpperCase();
...
export class CurrencyRatesService {
  ...
  private ruleFromProductCurrencyRate(
    rule: ProductCurrencyRate,
  ): CurrencyRateRuleConfig {
    return {
      buy: {
  ...
  async createGroup(
    dto: CreateCurrencyRateGroupDto,
    userId: string,
  ): Promise<CurrencyRateGroup> {
    const code = dto.code.trim().toUpperCase();
    const existing = await this.groupRepository.findOne({ where: { code } });
    if (existing) {
      throw new ConflictException(
        `Currency rate group with code "${code}" already exists`,
      );
    }
  ...
  async updateGroup(
    id: string,
    dto: UpdateCurrencyRateGroupDto,
    userId: string,
  ): Promise<CurrencyRateGroup> {
    const group = await this.groupRepository.findOne({ where: { id } });
    if (!group) {
      throw new NotFoundException(
        `Currency rate group with id ${id} not found`,
      );
    }
  ...
  async createRateEntry(
    ...
    if (
      dto.provider === CurrencyRateProvider.FOREX ||
      dto.provider === CurrencyRateProvider.MANUAL
    ) {
      if (baseRate) {
        ...
        throw new BadRequestException(
          "baseRate or baseBuyRate/baseSaleRate is required for FOREX and MANUAL providers",
        );
      ...
      if (!baseBuyRate || !baseSaleRate) {
        throw new BadRequestException(
          "baseBuyRate and baseSaleRate are required for TICKER provider",
        );
      }
      baseRate = null;
    }
  ...
  async createProductCurrencyRate(
    ...
    if (existing) {
      throw new ConflictException(
        "Product currency pricing already exists for this product and currency",
      );
    }
    ...
    const rule = this.productCurrencyRateRepository.create({
      ...
      currency,
      currencyId: currency.id,
      buyMarginType: normalizeOptionalText(
        dto.buyMarginType,
      ) as CurrencyRateMarginType | null,
      ...
      saleMarginType: normalizeOptionalText(
        dto.saleMarginType,
      ) as CurrencyRateMarginType | null,
  ...
  async updateProductCurrencyRate(
    ...
    if (!rule) {
      throw new NotFoundException(
        `Product currency pricing with id ${id} not found`,
      );
    }
    ...
    if (dto.currencyId !== undefined && dto.currencyId !== rule.currencyId) {
      ...
      const duplicate = await this.productCurrencyRateRepository.findOne({
        where: {
          productId: dto.productId ?? rule.productId,
    ...
    if (dto.buyMarginType !== undefined)
      rule.buyMarginType = normalizeOptionalText(
        dto.buyMarginType,
      ) as CurrencyRateMarginType | null;
    if (dto.buyMarginValue !== undefined)
      rule.buyMarginValue = normalizeOptionalText(dto.buyMarginValue);
    if (dto.buyMinRate !== undefined)
      rule.buyMinRate = normalizeOptionalText(dto.buyMinRate);
    if (dto.buyMaxRate !== undefined)
      rule.buyMaxRate = normalizeOptionalText(dto.buyMaxRate);
    if (dto.saleMarginType !== undefined)
      rule.saleMarginType = normalizeOptionalText(
        dto.saleMarginType,
      ) as CurrencyRateMarginType | null;
    if (dto.saleMarginValue !== undefined)
      rule.saleMarginValue = normalizeOptionalText(dto.saleMarginValue);
    if (dto.saleMinRate !== undefined)
      rule.saleMinRate = normalizeOptionalText(dto.saleMinRate);
    if (dto.saleMaxRate !== undefined)
      rule.saleMaxRate = normalizeOptionalText(dto.saleMaxRate);
  ...
  async findProductCurrencyRates(
    productId?: string,
    currencyId?: string,
  ): Promise<ProductCurrencyPricingRule[]> {
    const qb = this.productCurrencyRateRepository
  ...
  private toProductCurrencyPricingRule(
    entity: ProductCurrencyRate,
  ): ProductCurrencyPricingRule {
    return {
      id: entity.id,
  ...
  private applyMargin(
    ...
    const amount =
      margin.marginType === CurrencyRateMarginType.PERCENT
  ...
  private validateBand(
    ...
    if (value < min) {
      return {
        isValid: false,
        reason: `Final rate ${finalRate} is below minimum ${margin.minRate}`,
      };
    }
    if (value > max) {
      return {
        isValid: false,
        reason: `Final rate ${finalRate} is above maximum ${margin.maxRate}`,
      };
    }
    return { isValid: true };
  }
  ...
  private async resolvePricingRule(
    ...
    if (!group) {
      throw new NotFoundException(
        `Currency with id ${currencyId} is not assigned to a pricing group`,
      );
    }
    ...
    if (!groupRule.buy || !groupRule.sale) {
      throw new NotFoundException(
        `No pricing rule found for currency group "${groupCode || "N/A"}"`,
      );
    }
  ...
  async previewQuote(dto: PreviewCurrencyRateDto): Promise<CurrencyRateQuote> {
    const { product, currency, rule, source, groupCode } =
      await this.resolvePricingRule(dto.productId, dto.currencyId);
    ...
    if (!baseBuyRate || !baseSaleRate) {
      throw new BadRequestException(
        "Base buy and sale rates are required for preview",
      );
    }
